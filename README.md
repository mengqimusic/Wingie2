...
4. 连接成功后，页面读取一次完整设备快照，之后每秒在后台轻量轮询：平时只重读状态与设置，
   仅当 Ratio profile 的 revision 变化时才重读 Ratio 全量、当某侧 Cave bank 的 revision 或
   active bank 变化时才重读该 bank；对 status 响应中尚不提供 Cave revision 的旧固件，自动
   回退为每秒全量重读。有效编辑会立即写入设备并作用于当前运行状态，不需要 Apply。编辑、
   实时写入和 Save 期间自动跳过后台轮询；
5. 若实体控件、MIDI 或其他软件改变了设备，后台轮询会在空闲时把变化同步到页面；写入会等待
   进行中的轮询结束，轮询结果落地前也会复查是否有更新的写入，合入时 revision 只进不退，
   不会用旧快照覆盖较新的本地状态。若写入遇到 revision 冲突，页面会自动重读对应资源、保留
   未提交的编辑，并以新 revision 重试一次，只有再次冲突才报错。也可以点击 Refresh 立即重读；
...
polls lightly once per second in the background: it normally re-reads only status and settings,
re-reading the full Ratio profile only when its revision changes and a Cave bank only when that bank's
revision or the active bank changes; on older firmware whose status response does not report Cave
revisions, it falls back to re-reading the full snapshot every second. Every valid edit is written to
the running instrument immediately, with no Apply step, and background polling is skipped while edits,
live writes, or Save are in flight. If hardware controls, MIDI, or other software changes the
instrument, the next idle poll brings the page up to date: a write waits for any poll already in
flight, polled data is re-checked for newer writes before being applied, and merging never rolls a
revision back, so a stale snapshot cannot overwrite newer local state. On a revision conflict the page
automatically re-reads the affected resource, keeps any unsubmitted edits, and retries once with the
new revision; only a second conflict is reported as an error. Use Refresh to re-read immediately, and
the confirmed “Save to Flash” action only when the current persistent settings should be written to
flash.
...
范围是 Zone、逐音 ownership、Pitch Bend，以及 Channel Pressure（0xD0）衰减（复音、比例模式每音最多 2 秒、一侧最多 6 秒；琴弦、音块与常规整侧单音 6 秒）；CC 74 不映射。
