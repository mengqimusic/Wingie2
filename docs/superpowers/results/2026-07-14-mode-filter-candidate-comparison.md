...
Coupled D、block-rate rotation/modal bank、normalized-lattice E 和 normalized-lattice R。它们都已有完整候选
...
当前正常固件证据中，Q、E、R 有用户听感原话；block-rate direct form 已复测确认有
wet，D 与 modal bank 已确认正常工作。后三项没有进一步的音色偏好、完整压力或逐路径
记录。E 有可归属的部分 MIDI 压力记录；R 没有完整压力矩阵；E/R 都没有足以标记 PASS
的 mute/unmute 真机记录。本轮多次切换候选后，最终留在设备上的镜像没有记录。

另外两条路径不在当前试听池：历史 direct form 是产品基线而不是新候选；直接
`fi.tf2np` 在固定 Faust 2.59.6 和真实 alternate-tuning 图下不能完成生成，因此没有可上机
的完整候选。block-rate direct form 早期压力实验中的“所有通道都没有 wet”仍保留为
历史观察，但同一高速变 pitch 压力已在历史 direct form 上分别触发左右 wet/resonator
停止；后续正常固件复测已有 wet，因此不能把旧现象归为候选静态缺少 wet。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1. 历史 `pm.modeFilter` direct form | 每模态二阶 direct-form 延迟状态 | 频率、T60 与递归均为原有 sample-rate 图 | 固定系数响应为历史基准 | 旧延迟状态直接套用新系数；没有独立收缩证明 | 是，alternate tuning 完整 | 是；当代 complete p99 `715.9 us`、0 miss | 历史恢复后左右 wet 经确认正常 | 保留为 main 产品源基线 | 基线，不是替换候选 |
| 2. Block-rate decay direct form | 与历史 direct form 相同 | 仅 decay pole-radius 每 32 samples 准备；其余保持原图 | settled 与动态矩阵近似历史等价 | 状态坐标不变，只降低 `pow` 成本 | 是 | 是；complete p99 `456.3 us`、0 miss | 高速 MIDI 压力后无 wet；2026-07-14 正常固件复测确认有 wet | 历史实验因当时物理现象拒绝并恢复基线 | 当前可试听；旧现象符合继承的 direct-form 压力故障 |
...
| 4. Coupled Q | 每模态 `rho R(theta)` 二维收缩旋转；输出缩放 quadrature state | `rho` 每 32 samples；状态逐 sample | 保留主要峰和目标 pole，不是历史 numerator 等价 | 旧二维状态在新旋转/收缩系数下继续，不 reset | 是，alternate tuning 完整 | 是；complete p99 `714.6 us`、0 miss | 2026-07-14 正常固件已试听：`我听起来没问题。` | 旧 p99 门禁拒绝，后续正常固件曾被跳过 | 保留；未安装到 main |
| 5. Coupled D | 与 Q 同一核心，quadrature 输出再做一拍差分 | 与 Q 相同，另有逐 sample 差分记忆 | 同 pole/主要峰目标；差分改变低频和增益，不是历史严格等价 | 核心状态连续，输出差分记忆也连续 | 是，alternate tuning 完整 | 是；complete p99 `715.2 us`、0 miss | 2026-07-14 正常固件已确认正常工作 | 旧 p99 门禁拒绝并跳过正常固件 | 当前可试听；音色偏好与完整压力未记录 |
| 6. Block-rate rotation/modal bank | 独立 `rho R(theta)` 二维 rotation bank | `rho/sin/cos` 与标定每 32 samples；状态逐 sample | 单模态一个主要窄峰；允许不同于历史响应 | 任意 note/ratio 跳变后，原状态在下一 block 的新旋转下继续 | 是，alternate tuning 完整 | 是；complete p99 `717.1 us`、0 miss | 2026-07-14 正常固件已确认正常工作 | 旧 p99 门禁拒绝；instrumented log 曾记录 `mode_changed` 路径差异 | 当前可试听；路径差异未阻断本次正常运行 |
...
| 13 | Image size / checksum / build | 通过：历史正常镜像 | 通过：benchmark、normal、diagnostic | 不适用 | 通过：benchmark；后来补建正常镜像 | 通过：benchmark 与正常镜像 | 通过：benchmark 与正常镜像 | 通过：generated、normal、complete images | 通过：generated 与 normal images |
| 14 | Flash-address / recovery integrity | 通过：历史 app0 多次 verify/readback | 通过：候选写入及历史恢复 | 不适用 | 通过：benchmark 恢复及正常镜像写入 | 通过：benchmark、恢复及正常镜像写入 | 通过：两阶段写入、恢复及正常镜像写入 | 通过：app0 恢复与 segmented-failure readback 相等 | 部分：正常镜像已刷写；无可归属的 R recovery readback |
...
| 16 | Wet/dry path | 部分：恢复后左右 wet 正常，dry 未单列 | 部分：当前复测确认有 wet；旧高速压力触发继承故障 | 不适用 | 未测：只有总体试听原话 | 部分：用户确认正常工作，路径未分项 | 部分：用户确认正常工作，路径未分项 | 通过：左右 wet 与 dry/thru 正常 | 通过：左右 wet 与 dry/thru 正常 |
...
| 18 | Physical runnability（当前唯一淘汰门禁） | 通过：历史正常固件/恢复镜像可运行 | 通过（当前）：正常固件有 wet；旧高速压力故障另记 | 不适用：无完整候选可上机 | 通过：complete benchmark 与正常固件 | 通过：complete benchmark 与正常固件 | 通过：complete benchmark 与正常固件；旧 UI log 另记 | 通过：正常固件及两个 complete captures | 通过：正常固件；complete timing 未测 |
| 19 | User listening | 部分：只确认恢复 wet 正常，无候选 A/B | 部分：确认有 wet，未给音色判断 | 不适用 | 通过：`我听起来没问题。` | 部分：确认正常工作，未给音色判断 | 部分：确认正常工作，未给音色判断 | 通过：`E 也非常好，而且音量明显比 R 要高。` | 通过：`听起来没问题` |
| 20 | Installation boundary | 已安装基线：main 仍是此 product source | 未安装：archive 中已 revert | 未安装 | 未安装 main；正常 Q 仅用于设备试用 | 未安装 | 未安装 | 未安装 | 未安装 |

矩阵中的第 18 项决定当前试听资格：BR-DF 当前复测有 wet；TF2NP 没有可测试的完整候选；
BR-DF、Q、D、Modal、E、R 均有相应范围内的正常固件硬件运行证据。其余
...
p99/max 降到 `456.3/469.642 us`，0 miss。原实验在 String channel 2 压力后曾收到
`当前所有通道都没有 wet` 的报告，因此按当时门禁停止、revert 并跳过 Poly/Bar 与 A/B。
MIDI counters 和 marker 正常，旧记录没有确定根因。

更早的历史 direct-form 诊断已经证明：String mode 下约 1 kHz 连续变 pitch 可分别令
被测侧 wet/resonator 停止，同时 parser、callback、Faust pitch 参数、另一侧 wet 与
dry/thru 继续工作。block-rate 候选保留同一 direct-form 递归状态和快速换频路径；原实验
在 channel 1 的 100,000 对消息后仍确认全部 wet 正常，在 channel 2 的 100,000 对消息后
才报告全部 wet 消失。因此高速变 pitch MIDI 压力是有证据支持的触发条件，但该轮不能
解释成左右两侧依次独立失效。内部状态与全 wet 消失的传播机制没有直接 probe，仍不能
写成 NaN/Inf 等更具体根因。

2026-07-14 使用同一冻结候选重建正常固件后，用户确认 block-rate-direct 有 wet。因此
旧现象应归入继承的高速 MIDI 压力故障，不是候选静态缺少 wet，也不再淘汰该候选。当前
证据只确认 wet 存在；左右/dry 分项、压力复测和音色判断仍未记录。
...
正常镜像，用户评价 `我听起来没问题。` 没有新的完整压力矩阵，也没有修改 main 产品源。
...
0 miss。2026-07-14 又构建了 1,191,216-byte 正常镜像，SHA-256 为
`69a04121e3fb6f69f0a4e8b0072ad14b01332fe4d50a541cd1fcda82dfbf3e05`；用户确认正常工作。
这建立正常 build/flash/run 证据，但不自动补成逐路径 wet/dry、压力或音色偏好结论。
...
2026-07-14 又构建了 1,189,504-byte 正常镜像，SHA-256 为
`9f381eaa28592f8cd4636530d0203444c8fd4a52542844c1797fa92a68585f64`；用户确认正常工作。
因此旧 UI log 没有阻断本次正常运行，但仍不能反向证明每个 `mode_changed` 控制路径都
已逐项覆盖；本次也没有单独的音色偏好或完整压力结论。
...
“听起来没问题”。block-rate direct form 已确认有 wet，D 和 modal 已确认正常工作，但
三者都没有进一步的音色偏好。除此之外，不存在足以支持
...
| Block-rate decay direct form | `419.1 / 424.071 us` | 0 | `456.3 / 469.642 us` | 0 | 旧时序门禁通过；当前有 wet，继承高速 MIDI 压力故障 |
...
| --- | --- | --- | --- | --- | --- | --- |
| Block-rate direct | 有，0 miss | 有 | 确认有 wet；路径未分项 | 未复测 | 未给音色判断 | 左右/dry、压力、mute 与音色判断 |
| Q | 有，0 miss | 有 | 未单独记录 | 未形成新的完整矩阵 | `我听起来没问题。` | wet/dry、系统压力、分模式与 mute 记录 |
| D | 有，0 miss | 有 | 正常工作；路径未分项 | 未测 | 未给音色判断 | 逐路径、压力、mute 与音色判断 |
| Modal bank | 有，0 miss | 有 | 正常工作；路径未分项 | 未测 | 未给音色判断 | `mode_changed` 分项、压力、mute 与音色判断 |
...
这张表只整理已发生的事实，不替用户决定下一台正常固件。六个可生成候选都已运行正常
固件；其中 Q、E、R 有明确听感原话，block-rate direct、D、modal 目前只确认 wet 或正常
工作。最终选择任一候选或都不选，当前记录均无决定。
...
form 继承高速 MIDI 变 pitch 压力故障、modal 的旧 instrumented log 有 UI 路径差异、E 有输出归一化放大、R 缺 complete timing，
并精确限定下一次真机试用需要观察什么。
