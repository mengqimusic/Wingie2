...
- 刷写前读取并保存完整 flash；测试完成后恢复原始 flash。
...
刷写前保存完整 flash 镜像及 flash ID。诊断固件通过完整 Arduino 编译后才上传。测试结束后写回原始 flash，并通过 Wingie2 启动日志、保存的 MIDI 通道及 tuning 状态确认恢复。仓库中的诊断改动不与后续产品修复混在同一提交。
