...
- 固件把 `<`、完整 JSON 与换行作为一次串口写入，避免其他 task 的调试日志插入协议帧。
...
control task 完成硬件初始化与 Preferences 加载前，包括 `hello` 在内的所有协议请求都返回
`busy`。网页必须重试 `hello`，不能在收到成功握手前读取或保存默认 RAM 值。
...
Ratio `revision` 在每次成功的 `set` 或 `reset` 后递增；Cave bank `revision` 在成功的
`set_cave` 后递增。`save` 只改变 dirty 状态，不改变运行配置，因此不递增 revision。
网页可在写入命令中携带 `expected_revision`，用于拒绝两个配置页面同时覆盖彼此修改的情况。
...
`frequency_min` 与 `frequency_max` 是 Ratio Mode DSP 的实际频率边界，网页用它们显示
裁剪后的有效频率。示例中的限制值是协议形状，不冻结产品参数；实现前必须由设计决定
替换或确认。
...
4. `hello` 成功后读取 `get` 和左右共六个 Cave bank，使用设备返回的 `limits` 建立输入约束；
