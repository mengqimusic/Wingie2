...
从 resonator 滤波器算法本身修复高速 MIDI 换音后单侧 wet 永久无声的问题。第一候选在 `Wingie2.dsp` 内版本化新版 `tf2np` 的 protected normalized-ladder biquad 公式，保持现有 MIDI/control 语义和静态 modal 参数映射，同时继续用 Faust 2.59.6 生成产品代码。
...
Faust 2.59.6 标准库中的 `fi.tf2np` 使用固定 `smax = 0.9999`。在 44.1 kHz 下，它会把最大有效 T60 限制在约 3.1326 seconds：请求 5 seconds 时误差约 -37.35%，请求 10 seconds 时误差约 -68.67%。因此本项目不能直接调用该旧实现。

候选把新版 protected normalized-ladder 公式作为本地 `protectedTf2` 写入 `Wingie2.dsp`。它使用 `smax = 1.0 - ma.EPSILON`，其余结构与当前 Faust `tf2np` 相同：

```faust
protectedTf2(b0, b1, b2, a1, a2) =
  fi.allpassnnlt(M, sv) : sum(i, M + 1, *(tghr(i)))
with {
  smax = 1.0 - ma.EPSILON;
  s2 = max(-smax, min(smax, a2));
  s1 = max(-smax, min(smax, a1 / (1 + a2)));
  sv = (s1, s2);
  M = 2;
  nu(2) = b2;
  nu(1) = b1 - b2 * a1;
...
`stableModeFilter` 保持相同的 numerator、pole radius、frequency、T60 和 gain，只把 direct-form `fi.tf2` 替换为本地 `protectedTf2`：

```faust
stableModeFilter(freq, t60, gain) = protectedTf2(1, 0, -1, a1, a2) * gain
with {
  w = 2 * ma.PI * freq / ma.SR;
  rho = pow(0.001, 1.0 / (t60 * ma.SR));
  a1 = -2 * rho * cos(w);
...
`protectedTf2` 把 denominator 转成 normalized-ladder reflection coefficients，并把它们投影到定义的稳定区域。它比 direct-form `tf2` 更适合时变系数，但会引入 `sqrt(1-s*s)` 和额外的 scattering/tap 运算。公式旁必须保留 Faust/Julius O. Smith III 来源与原 MIT-style STK license attribution。
...
1. 直接使用本地 `protectedTf2`，其公式与当前 Faust `tf2np` 一致，且使用 `1.0 - ma.EPSILON` stability projection。
2. 只有本地直接版本未通过 CPU 门禁时，停止并记录结果，再设计 Wingie 专用 normalized-ladder mode filter，通过共享 `rho`、`a2/s2` 等值减少计算。
...
若 parity 失败，先解决或固定生成工具链。本 spec 不升级 Faust，也不允许用当前 Faust 2.85.5 产生的大范围 architecture/library churn 掩盖单一滤波器修改。Faust 2.85.5 migration 必须另立 spec、plan 和真机 A/B。
...
只有下列条件全部满足，本地 protected normalized-ladder 候选才可进入产品分支：
