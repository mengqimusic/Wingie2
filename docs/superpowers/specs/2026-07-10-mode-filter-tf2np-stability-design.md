...
从 resonator 滤波器算法本身修复高速 MIDI 换音后单侧 wet 永久无声的问题。第一候选使用 Faust 2.59.6 `filters.lib` 中的 `fi.tf2np` protected normalized-ladder biquad，保持现有 MIDI/control 语义和静态 modal 参数映射。
...
Faust 2.59.6 同时带有 deprecated `filter.lib`（单数）和 namespaced `filters.lib`（复数）。单数兼容库的旧 `tf2np` 使用 `smax = 0.9999`，但 Wingie 通过 `stdfaust.lib` 的 `fi` namespace 使用复数库；该版本的 `fi.tf2np` 已使用 `smax = 1.0 - ma.EPSILON`，不会产生 3.13-second T60 上限。工具链门禁必须检查复数 `filters.lib`，不能用单数库判断实现。
...
1. 直接使用 Faust 2.59.6 `filters.lib` 的 `fi.tf2np`。
