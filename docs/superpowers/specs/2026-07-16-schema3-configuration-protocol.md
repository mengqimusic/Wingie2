...
`3`。Schema 3 在既有 Ratio/Cave 命令之外增加 `get_settings` 与 `set_param`；网页使用已有读取
命令进行后台完整快照轮询，不增加设备主动推送。
...
`hello` 返回 `busy`，网页只重试握手，最长等待约 `5 s`；这段重试不会读取配置。连接成功后网页
启动每秒一次的完整快照轮询，编辑、实时写入、Save 或手动 Refresh 期间跳过轮询。
...
连接成功后网页每秒重复同一套完整快照读取。后台读取不会清除或覆盖正在编辑的草稿，并且不锁定
编辑框；用户点击 Refresh 时，网页先提交并等待已排队的编辑写入，再重复同一套完整快照读取。

因此，实体控件或 MIDI 在快照之后造成的设备变化会在下一次空闲后台快照中回写网页；也可以手动
Refresh 立即重读。`set_param` 返回 `caves_changed: true` 是本次网页写入的因果结果，网页会
立即重读 6 个 Cave bank。
...
- `mpe_enabled` 表示是否在开机时建立 Lower→Left、Upper→Right 的双 Zone；运行时 MCM 不改变该持久化值；
...
| `mpe_enabled` | 四舍五入为整数后裁剪到 `0..1` | 是 | `false` |
...
网页使用现有 `get_settings`、`get` 和 `get_cave` 命令每秒读取完整快照；这是客户端定时请求，
不是协议事件、heartbeat 或设备主动推送。编辑、实时写入、Save 或手动 Refresh 期间跳过后台读取。
