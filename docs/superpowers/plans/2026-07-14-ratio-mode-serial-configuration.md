# Ratio Mode 与纯 HTML 串口配置实施计划

## 目标

新增一个可由键盘和 MIDI 演奏的 Ratio Mode。左右声道共用一个九 ratio profile，ratio
允许低于 `1`，不提供逐共鸣器 mute。配置使用可直接植入现有 HTTPS 网页的单文件 HTML，
通过浏览器 Web Serial API 连接 USB 串口；不在 ESP32 上加入 SoftAP、HTTP server 或
网页资源。
...
- `Wingie2/ratio_serial.ino`：固定 schema 的串口 framing、请求队列、响应和错误；
- `Wingie2/ratio_profile.h`：profile 数据、范围元数据、校验和与版本；
- `Tools/ratio_mode.html`：单文件 HTML/CSS/JavaScript，直接使用 Web Serial；
- `tests/host/ratio_profile_test.cpp`：profile 校验、版本、原子替换和持久化编码测试；
- `tests/host/ratio_serial_protocol_test.cpp`：framing、请求/响应和错误 fixture；
...
- `Wingie2/Wingie2.ino`：加入 `RATIO_MODE`、第五模式 LED 状态、共享 profile 全局状态，
  保持 Wi‑Fi 关闭；
- `Wingie2/MIDI.ino`：让 Ratio Mode 复用 String/Bar 的 Note On 路径，并拒绝非法 mode；
- `Wingie2/control.ino`：加载 profile、应用 ratio、处理第五模式闪烁和串口 service；
- `Wingie2/stuff.ino`：把共享 profile 纳入现有保存流程；
...
先完成 `ratio_profile.h` 与 host tests，再接入 DSP。合同至少包含：
...
`Tools/ratio_mode.html` 必须是一个可独立复制的文件：
...
- 一个共享的九行 ratio 表，不显示左右重复编辑器；
- 显示 ratio、cents、左右当前 effective Hz；
- 提供 Apply/Save/Reset、dirty 状态、Export/Import；
...
1. hello/get 返回共享 profile 和 limits；
2. set 九值后 Ratio Mode 在下一个 block 使用新 profile；
3. standard/alternate tuning 下左右键盘与 MIDI Note On 都能移调；
4. ratio 小于 `1`、接近上下限、快速 note/ratio 变化均保持 wet、finite 和稳定；
5. reset 不写 flash，save 后断电重启仍保留 profile；
6. Save、mode change、Ratio blink、wet/dry、decay 和 anti-feedback 不互相破坏；
7. 退出 Ratio Mode 后 Poly/String/Bar/Cave 和原有 preferences 不回归。
...
4. `feat(web): 添加纯 HTML Web Serial 配置页`；
