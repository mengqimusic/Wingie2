...
普通用户推荐直接打开单文件 `Wingie2-v4.02.standalone.html`。它已经内嵌 manifest、四段固件和固定版本浏览器依赖，不需要选择固件包，也不会下载同目录资源。Squarespace 只放一个跳转到该 HTTPS 页面的按钮；不要把刷机页放进 iframe 或 Code Block。
...
2. 从 HTTPS 地址打开 `Wingie2-v4.02.standalone.html`，点击“连接设备”。浏览器只会在用户选择端口后授权访问。
