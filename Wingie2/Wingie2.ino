...
void unmute_channel_resonators(byte ch) {
  for (uint8_t index = 0; index < 9; index++) cm_mute_set(ch, index, false);
}
...
void apply_pitched_mode_channel(byte ch, int midiNote) {
  ...
  for (uint8_t index = 0; index < 9; index++) {
    const float frequency = max(16.0f, min(16000.0f,
...
void tune_caves() {
  ...
  for (byte ch = 0; ch < 2; ch++) {
    if (Mode[ch] == CAVE_MODE) apply_cave_bank_to_dsp(ch, oct[ch] + 1);
  }
