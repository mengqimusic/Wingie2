...
static const float kCaveFrequencyMax = 15999.99f;
...
inline bool canonicalizeRatio(float value, float &canonical) {
  if (!finite(value)) return false;
  if (value < kRatioMin) value = kRatioMin;
  if (value > kRatioMax) value = kRatioMax;
  canonical = static_cast<float>(lroundf(value * kRatioScale)) / kRatioScale;
  return true;
}
...
inline bool decodeRatioProfile(const RatioProfileStorage &storage, RatioProfileState &profile) {
  ...
  for (uint8_t index = 0; index < kRatioCount; index++) {
    ...
    decoded.ratios[index] = static_cast<float>(storage.ratiosMilli[index]) / kRatioScale;
  }
