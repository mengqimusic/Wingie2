...
class RatioModeReferenceTest(unittest.TestCase):
    ...
    def test_per_voice_expression_parameters_match_contract(self):
        ...
        self.assertIn("wingie_decay::kPressureDepthPerVoiceSeconds", mpe)
        self.assertIn("wingie_decay::kPressureDepthMonoSeconds", mpe)
        self.assertIn("set_fader_decay(0, Decay);", control)
        self.assertIn("set_fader_decay(0, v);", midi)
        serialcfg = (REPO_ROOT / "Wingie2/serial_config.ino").read_text(encoding="utf-8")
        self.assertIn("set_fader_decay(ch, canonical);", serialcfg)
        ...
        self.assertIn("wingie_decay::kPressureDepthMonoSeconds", firmware)
